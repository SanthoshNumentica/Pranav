<template>
  <div class="max-w-5xl mx-auto space-y-8 pb-20">
    <!-- Page Header -->
    <div class="flex items-center justify-between animate-in fade-in slide-in-from-top-4 duration-500">
      <div>
        <h1 class="text-3xl font-bold text-slate-900 tracking-tight">New Case Report</h1>
        <p class="text-slate-500 mt-2 font-medium">Register a new diagnostic case and upload DICOM files.</p>
      </div>
      <button 
    ...
    <form @submit.prevent="handleSubmit" class="grid grid-cols-1 lg:grid-cols-3 gap-8">
      ...
      <div class="lg:col-span-1 space-y-8 animate-in fade-in slide-in-from-left-4 duration-700 delay-100">
        <div class="bg-white rounded-[32px] border border-slate-200 p-8 shadow-soft-xl space-y-6">
          ...
          <!-- Patient Selection -->
          <div class="space-y-2">
            <label class="text-[10px] font-bold uppercase tracking-[0.2em] text-slate-400 ml-1">Patient</label>
            <div class="relative">
              <select 
              ...
              <ChevronDownIcon class="absolute right-4 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-400 pointer-events-none" />
            </div>
          </div>

          <!-- Doctor Selection -->
          <div class="space-y-2">
            <label class="text-[10px] font-bold uppercase tracking-[0.2em] text-slate-400 ml-1">Referring Doctor</label>
            <div class="relative">
              <select 
          ...
          <div class="space-y-2">
            <label class="text-[10px] font-bold uppercase tracking-[0.2em] text-slate-400 ml-1">Case Documents (JPG, PNG)</label>
            <div class="flex items-center gap-3">
              ...
              >
                ...
                <template v-if="!processingGeneral">
                  <PaperclipIcon class="h-4 w-4 text-slate-400 group-hover:text-primary transition-colors" />
                  <span class="text-xs font-bold text-slate-500 group-hover:text-primary transition-colors">Attach Files</span>
                </template>
                <template v-else>
            ...
            <div v-if="form.documents.length > 0" class="flex flex-wrap gap-2 mt-2">
              ...
              >
                <div class="h-4 w-4 rounded-md bg-white border border-slate-200 flex items-center justify-center">
                  <CheckIcon class="h-2.5 w-2.5 text-emerald-500" />
                </div>
                <span class="truncate max-w-[80px]">{{ doc.name }}</span>
                <button type="button" @click="removeGeneralDoc(dIdx)" class="hover:text-rose-500 transition-colors">
                  <XIcon class="h-3 w-3" />
                </button>
              </div>
            </div>
          ...
          <div class="space-y-2">
            <label class="text-[10px] font-bold uppercase tracking-[0.2em] text-slate-400 ml-1">Description (Optional)</label>
            <textarea 
      ...
      <div class="lg:col-span-2 space-y-8 animate-in fade-in slide-in-from-right-4 duration-700 delay-200">
        <div class="bg-white rounded-[40px] border border-slate-200 p-8 md:p-10 shadow-soft-xl">
          <div class="flex items-center justify-between mb-8">
            <div class="flex items-center gap-3">
              <div class="h-10 w-10 rounded-2xl bg-primary/10 flex items-center justify-center">
                <ActivityIcon class="h-5 w-5 text-primary" />
              </div>
              <h3 class="font-bold text-lg text-slate-900">Scan Items & Documents</h3>
            </div>
            <button 
              type="button"
              @click="addItem"
          ...
          <div class="space-y-6">
            ...
            >
              <!-- Index Badge -->
              <div class="absolute -left-3 top-6 h-8 w-8 rounded-full bg-slate-900 text-white flex items-center justify-center text-[10px] font-bold shadow-lg">
                {{ index + 1 }}
              </div>
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-4">
                  <div class="space-y-2">
                    <label class="text-[10px] font-bold uppercase tracking-[0.2em] text-slate-400 ml-1">Scan Type</label>
                    <div class="relative group/select">
                      <div class="absolute left-4 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-300 group-hover/select:text-primary transition-colors">
                      ...
                      >
                        ...
                        <option v-for="type in scanTypes" :key="type.id" :value="type.id">
                          {{ type.name }}
                        </option>
                      </select>
                      <ChevronDownIcon class="absolute right-4 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-400 pointer-events-none group-hover/select:text-primary transition-colors" />
                    </div>
                  </div>

                  <div class="space-y-2">
                    <label class="text-[10px] font-bold uppercase tracking-[0.2em] text-slate-400 ml-1">Specific Scan</label>
                    <div class="relative group/select">
                      <div class="absolute left-4 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-300 group-hover/select:text-primary transition-colors">
                        <ActivityIcon class="h-4 w-4" />
                      </div>
                      ...
                      >
                        ...
                        <option v-for="scan in getScans(item.scan_type_id)" :key="scan.id" :value="scan.id">
                          {{ scan.name }}
                        </option>
                      </select>
                      <ChevronDownIcon class="absolute right-4 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-400 pointer-events-none group-hover/select:text-primary transition-colors" />
                ...
                <!-- File Collection -->
                <div class="space-y-2">
                  <label class="text-[10px] font-bold uppercase tracking-[0.2em] text-slate-400 ml-1">DICOM Files (.dcm)</label>
                  <div 
                  ...
                  >
                    ...
                    <div class="flex flex-col items-center gap-2 pointer-events-none">
                      <div class="p-2 rounded-xl bg-slate-50 group-hover/upload:bg-primary/10 transition-colors">
                        <UploadIcon v-if="!item.processing" class="h-5 w-5 text-slate-400 group-hover/upload:text-primary" />
                        <Loader2Icon v-else class="h-5 w-5 text-primary animate-spin" />
                      </div>
                      <span class="text-[11px] font-bold text-slate-500 group-hover/upload:text-primary">
                        {{ item.processing ? 'Analyzing files...' : 'Click or drag DICOM files' }}
                      </span>
                    </div>
                  </div>

                  <!-- File List Preview -->
                  <div v-if="item.documents.length > 0" class="mt-3 flex flex-wrap gap-2">
                    <div 
                    ...
                    >
                      ...
                      <button @click="removeDoc(index, dIdx)" class="hover:text-rose-500">
                        <XIcon class="h-3 w-3" />
                      </button>
                    </div>
                  </div>
          ...
          <!-- Empty State -->
          <div v-if="form.items.length === 0" class="py-12 border-2 border-dashed border-slate-100 rounded-3xl flex flex-col items-center justify-center gap-3">
            ...
            <p class="text-sm font-medium text-slate-400 italic">No scan items added yet.</p>
          </div>

          <!-- Final Actions -->
          <div class="mt-12 flex items-center justify-end gap-4 border-t border-slate-100 pt-8">
            <div v-if="error" class="text-xs text-rose-500 font-bold px-4 flex items-center gap-2 animate-in fade-in">
            ...
            >
              ...
              {{ loading ? 'Creating Case...' : 'Create Case Report' }}
            </button>
          </div>
        </div>
      </div>
...
const form = reactive({
  ...
  items: [
    { scan_type_id: '', scan_id: '', remarks: '', documents: [], processing: false }
  ]
});
...
const addItem = () => {
  form.items.push({ scan_type_id: '', scan_id: '', remarks: '', documents: [], processing: false });
};
