<template>
  <div class="space-y-6">
    ...
    <div class="bg-white rounded-3xl border border-slate-200 shadow-soft-xl overflow-x-auto custom-scrollbar">
      <table class="w-full">
        ...
        <tbody class="divide-y divide-slate-100">
          ...
          <template v-else>
            <tr v-for="(g, index) in filteredGenders" :key="g.id" class="group hover:bg-slate-50/50 transition-colors">
              ...
              <td class="px-6 py-4">
                <button @click="toggleStatus(g)" :class="cn(
                  ...
                  ">
                  <CircleIcon class="h-2 w-2 mr-1.5 fill-current" v-if="g.status === 'active'" />
                  {{ g.status }}
                </button>
              </td>
              <td class="px-6 py-4 text-xs font-medium text-slate-500">
                <div class="flex flex-col">
                  ...
                  <span v-if="g.added_by_user" class="text-[10px] text-slate-400">
              ...
              <td class="px-6 py-4 text-xs font-medium text-slate-500">
                <div class="flex flex-col">
                  ...
                  <span v-if="g.modified_by_user" class="text-[10px] text-slate-400">
                    by {{ g.modified_by_user?.name || "Unknown" }}
                  </span>
                  <span v-else-if="g.added_by_user" class="text-[10px] text-slate-400">
                    by {{ g.added_by_user?.name || "Unknown" }}
                  </span>
                </div>
              </td>
              <td class="px-6 py-4 text-right">
                <TableActions :item="g" :permissions="{ canView, canEdit, canDelete }" view-title="View"
    ...
    <!-- Dialog -->
    <MasterDataDialog :is-open="dialogOpen" :mode="dialogMode" title="Gender" label="Gender" :icon="GenderIcon"
      :initial-data="selectedGender" :loading="dialogLoading" :error="dialogError" @close="dialogOpen = false"
      @submit="handleDialogSubmit" />

    <!-- Confirmation Modal -->
    <ConfirmationModal :is-open="confirmOpen" title="Delete Gender"
      :description="`Are you sure you want to delete '${selectedGender?.gender_name}'?`" confirm-label="Delete"
