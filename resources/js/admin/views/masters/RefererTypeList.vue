<template>
  <div class="space-y-6">
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
      <div>
        ...
        <p class="text-sm text-slate-500 mt-1">
          Manage referer types (e.g., Hospital, Clinic, Agent).
        </p>
      </div>
        ...
        <tbody class="divide-y divide-slate-100">
          ...
          <template v-else>
            <tr v-for="(type, index) in filteredTypes" :key="type.id"
              ...
              <td class="px-6 py-4">
                <button @click="toggleStatus(type)" :class="cn(
                  ...
                  ">
                  <CircleIcon class="h-2 w-2 mr-1.5 fill-current" v-if="type.status === 'active'" />
                  {{ type.status }}
                </button>
              </td>
              <td class="px-6 py-4 text-xs font-medium text-slate-500">
                <div class="flex flex-col">
                  ...
                  <span v-if="type.added_by_user" class="text-[10px] text-slate-400">
              ...
              <td class="px-6 py-4 text-xs font-medium text-slate-500">
                <div class="flex flex-col">
                  ...
                  <span v-if="type.modified_by_user" class="text-[10px] text-slate-400">
                    by {{ type.modified_by_user?.name || "Unknown" }}
                  </span>
                  <span v-else-if="type.added_by_user" class="text-[10px] text-slate-400">
                    by {{ type.added_by_user?.name || "Unknown" }}
                  </span>
                </div>
              </td>
              <td class="px-6 py-4 text-right">
                <TableActions :item="type" :permissions="{ canView, canEdit, canDelete }" view-title="View"
            ...
            <tr v-if="filteredTypes.length === 0">
              <td colspan="6" class="px-6 py-20 text-center">
                <p class="text-slate-400 italic">
                  No referer types found matching your filter.
                </p>
              </td>
            </tr>
          </template>
    ...
    <!-- Dialog -->
    <MasterDataDialog :is-open="dialogOpen" :mode="dialogMode" title="Referer Type" label="Referer Type Name"
      :icon="BuildingIcon" :initial-data="selectedType" :loading="dialogLoading" :error="dialogError"
      @close="dialogOpen = false" @submit="handleDialogSubmit" />

    <!-- Confirmation Modal -->
    <ConfirmationModal :is-open="confirmOpen" title="Delete Referer Type"
      :description="`Are you sure you want to delete '${selectedType?.name}'?`" confirm-label="Delete" variant="danger"
      :loading="dialogLoading" @close="confirmOpen = false" @confirm="handleDeleteConfirm" />
  </div>
</template>
...
// Assuming 'referer-types' permission module exists or admins have access
const { canAdd, canView, canEdit, canDelete } =
  getModulePermissions("referer-types");
...
const fetchTypes = async (page = 1) => {
  loading.value = true;
  try {
    const response = await axios.get("/api/v1/masters/referer-types", {
      params: {
        page,
      },
    });
    if (response.data.success) {
      types.value = response.data.data.data;
...
const openEditDialog = (type) => {
  ...
  selectedType.value = { id: type.id, name: type.name };
  dialogError.value = null;
  dialogOpen.value = true;
};
...
const handleDialogSubmit = async (formData) => {
  ...
  try {
    ...
    if (dialogMode.value === "add") {
      response = await axios.post("/api/v1/masters/referer-types", {
        name: formData.name,
      });
    } else {
      response = await axios.put(
        `/api/v1/masters/referer-types/${formData.id}`,
        {
          name: formData.name,
        },
      );
    ...
    if (response.data.success) {
      ...
      addToast({
        title: "Success",
        description: `Referer type ${dialogMode.value === "add" ? "created" : "updated"} successfully.`,
        variant: "success",
      });
    }
  } catch (e) {
...
const toggleStatus = async (type) => {
  ...
  try {
    const response = await axios.post(
      `/api/v1/masters/referer-types/${type.id}/status`,
      {
        status: newStatus,
      },
    );
    if (response.data.success) {
      ...
      addToast({
        title: "Success",
        description: "Referer type status updated successfully.",
        variant: "success",
      });
    }
  } catch (e) {
...
const handleDeleteConfirm = async () => {
  ...
  try {
    const response = await axios.delete(
      `/api/v1/masters/referer-types/${selectedType.value.id}`,
    );
    if (response.data.success) {
      await fetchTypes();
      ...
      addToast({
        title: "Success",
        description: "Referer type deleted successfully.",
        variant: "success",
      });
    }
  } catch (e) {
    ...
    addToast({
      title: "Error",
      description: "Failed to delete referer type.",
      variant: "error",
    });
  } finally {
    dialogLoading.value = false;
