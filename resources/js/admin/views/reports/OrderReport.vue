          ...
          <tbody class="divide-y divide-slate-50">
            ...
            <tr
              ...
              >
                ...
                <td class="px-3 py-4">
                  <span class="text-sm font-medium text-slate-900">{{ report.patient?.title?.title_name }} {{ report.patient?.name }}</span>
                </td>
                <td class="px-3 py-4">
                  <span class="text-sm text-slate-600">{{ report.referer?.name }}</span>
                </td>
...
const getStatusClass = (status) => {
  switch (status) {
    ...
    case 'pending': 
    case 'draft': return 'bg-amber-50 text-amber-600 border border-amber-100';
...
const exportToCSV = () => {
  ...
  link.setAttribute("download", `Case_Report_${filters.value.from_date}_to_${filters.value.to_date}.csv`);
