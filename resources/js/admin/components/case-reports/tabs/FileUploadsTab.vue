<template>
  <div class="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
    ...
    <div class="bg-white rounded-[32px] border border-slate-200 p-8 shadow-soft-xl space-y-6">
      ...
      <div class="space-y-2">
        ...
        <div class="flex items-center gap-3">
          <label
            ...
            <input type="file" multiple
              ...
              class="hidden" @change="handleGeneralFiles" :disabled="processingGeneral || !canEdit" />
            <template v-if="!processingGeneral">
              <PaperclipIcon class="h-4 w-4 text-slate-400 group-hover:text-primary transition-colors" />
              <span class="text-xs font-bold text-slate-500 group-hover:text-primary transition-colors">
                Attach Files
              </span>
            </template>
            <template v-else>
        ...
        <div v-if="form.documents.length > 0" class="flex flex-wrap gap-2 mt-2">
          <div v-for="(doc, dIdx) in form.documents" :key="dIdx"
            ...
            <button v-if="canEdit" type="button" @click="removeGeneralDoc(dIdx)"
    ...
    <!-- Scan Items -->
    <ScanItemsList class="bg-white rounded-[32px] border border-slate-200 p-8 shadow-soft-xl" :form="form"
      ...
      :get-unique-folders="getUniqueFolders" :can-edit="canEdit" />
  </div>
</template>
