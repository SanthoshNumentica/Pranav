  ...
  <div class="space-y-6">
    ...
    <div class="space-y-6">
      <div v-for="(item, index) in form.items" :key="index"
        ...
        <!-- Item Controls -->
        <button v-if="form.items.length > 1 && canEdit" type="button" @click="removeItem(index)"
            ...
            <div class="relative group/select">
              ...
              <Select :model-value="item.scan_types_fk_id" @update:model-value="(val) => onTypeChange(val, item)"
                ...
                <SelectContent class="rounded-2xl border-slate-100">
                  <SelectItem v-for="type in scanTypes" :key="type.id" :value="type.id.toString()">
                    {{ type.name }}
                  </SelectItem>
                </SelectContent>
              </Select>
        ...
        <div v-if="item.selected_scans && item.selected_scans.length > 0"
          class="mb-6 animate-in fade-in slide-in-from-top-2 duration-300">
          <div class="overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-sm">
            <table class="w-full text-left border-collapse">
              <thead>
                <tr class="bg-slate-50/50 border-b border-slate-100">
                  <th class="py-3 px-4 text-[10px] font-bold uppercase tracking-wider text-slate-400 w-12 text-center">#
                  </th>
                  <th class="py-3 px-4 text-[10px] font-bold uppercase tracking-wider text-slate-400">Description</th>
                  <th class="py-3 px-4 text-[10px] font-bold uppercase tracking-wider text-slate-400 text-right w-40">
                    Amount</th>
                  <th v-if="canEdit"
                    class="py-3 px-4 text-[10px] font-bold uppercase tracking-wider text-slate-400 w-12 text-center">
                  </th>
                </tr>
              </thead>
              <tbody>
                <tr v-for="(scan, sIdx) in item.selected_scans" :key="sIdx"
                  ...
                  <td class="py-3 px-4">
                    <div class="relative group/input flex justify-end">
                      ...
                      <input v-model="scan.amount" type="number" min="0" step="0.01" :disabled="!canEdit"
        ...
        <div class="space-y-4">
          <div v-if="showUpload" class="space-y-2">
            ...
            <div
              ...
              <input v-if="canEdit" type="file" webkitdirectory directory multiple
                class="absolute inset-0 opacity-0 cursor-pointer" @change="handleFiles($event, index)" />
              <div class="flex flex-col items-center gap-2 pointer-events-none" :class="{ 'opacity-50': !canEdit }">
                <div class="p-2 rounded-xl bg-slate-50 group-hover/upload:bg-primary/10 transition-colors">
                  <component :is="item.processing ? Loader2Icon : UploadIcon" :class="[
                    'h-5 w-5',
                    item.processing
            ...
            <div class="mt-2 space-y-2">
              <div v-for="folder in getUniqueFolders(item.documents)" :key="folder.name"
                ...
                <button v-if="canEdit" type="button" @click="removeFolder(index, folder.name)"
            ...
            <div v-if="getUniqueFolders(item.documents, true).length > 0" class="mt-3 flex flex-wrap gap-2">
              <div v-for="(doc, dIdx) in getUniqueFolders(item.documents, true)" :key="dIdx"
                ...
                <button v-if="canEdit" type="button" @click="removeDoc(index, dIdx)" class="hover:text-rose-500">
                  <XIcon class="h-3 w-3" />
                </button>
              </div>
            </div>
          ...
          <div class="space-y-2">
            ...
            <textarea v-model="item.remarks" rows="2" :disabled="!canEdit"
              ...
              placeholder="Special instructions or notes for this scan..."></textarea>
          </div>
        </div>
      </div>
    </div>
