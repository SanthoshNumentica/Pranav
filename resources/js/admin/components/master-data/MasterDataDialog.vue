                ...
                <div class="flex-1 overflow-y-auto p-6 sm:p-8 custom-scrollbar space-y-6">
                  ...
                  <slot :form="form" :mode="mode" :loading="loading">
                    <div class="space-y-1.5">
                      <label class="text-[10px] font-bold uppercase tracking-[0.2em] text-slate-400 ml-1">{{ label }}
                        Name</label>
                      <input v-model="form.name" type="text" :placeholder="`Enter ${label.toLowerCase()} name`"
                  ...
                  <!-- Audit Info -->
                  <div v-if="mode === 'view' && initialData"
                    class="pt-6 border-t border-slate-100 grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div class="space-y-1 bg-slate-50/50 p-3 rounded-xl border border-slate-100/50">
                      <span class="text-[10px] font-bold uppercase tracking-wider text-slate-400">Added By</span>
                      <p class="text-sm font-semibold text-slate-700">
                        {{ initialData.added_by_user?.name || "N/A" }}
                      </p>
                    </div>
                    <div class="space-y-1 bg-slate-50/50 p-3 rounded-xl border border-slate-100/50">
                      <span class="text-[10px] font-bold uppercase tracking-wider text-slate-400">Modified By</span>
                      <p class="text-sm font-semibold text-slate-700">
