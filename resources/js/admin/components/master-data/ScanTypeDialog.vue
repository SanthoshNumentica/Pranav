            ...
            <DialogPanel
              ...
              <div class="relative bg-primary px-6 py-8 sm:px-10 text-white overflow-hidden shrink-0">
                ...
                <div class="relative flex items-center justify-between">
                  ...
                  <button @click.stop="onClose" class="p-2 rounded-xl hover:bg-white/10 transition-colors">
              ...
              <form @submit.prevent="handleSubmit" class="flex flex-col flex-1 overflow-hidden">
                <div class="flex-1 overflow-y-auto p-6 sm:p-8 custom-scrollbar space-y-6">
                  ...
                  <div class="space-y-1.5">
                    <label class="text-[10px] font-bold uppercase tracking-[0.2em] text-slate-400 ml-1">Scan Type
                      Name</label>
                    <input v-model="form.name" type="text" placeholder="e.g. MRI, CT Scan"
                      ...
                      :disabled="mode === 'view' || loading" required />
                  </div>

                  <!-- Multiple Scans Field -->
                  <div class="space-y-3">
                    <div class="flex items-center justify-between">
                      <label class="text-[10px] font-bold uppercase tracking-[0.2em] text-slate-400 ml-1">Included
                        Scans</label>
                      <button v-if="mode !== 'view'" type="button" @click="addScan"
                    ...
                    <div class="space-y-2">
                      <div v-for="(scan, index) in form.scans" :key="index" class="flex items-center gap-2 group">
                        <div class="relative flex-[2]">
                          <input v-model="scan.name" type="text" placeholder="Scan Name"
                        ...
                        <div class="relative flex-1">
                          <input v-model="scan.amount" type="number" placeholder="Amount"
                            class="w-full bg-slate-50/50 border-slate-100 group-hover:bg-white group-hover:border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:ring-primary/20 focus:border-primary transition-all duration-200 disabled:opacity-75 disabled:cursor-not-allowed"
                            :disabled="mode === 'view' || loading" min="0" step="0.01" />
                        </div>
                        <button v-if="mode !== 'view'" @click="removeScan(index)" type="button"
                ...
                <div
                  ...
                  <button type="button" @click.stop="onClose"
