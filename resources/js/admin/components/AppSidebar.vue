<template>
  ...
  >
    <!-- Brand / Logo Area -->
    <div
      class="h-20 flex items-center px-6 gap-3 shrink-0 border-b border-slate-100"
    >
      <div
      ...
      <div
        v-if="!isCollapsed"
        class="flex flex-col overflow-hidden animate-in fade-in slide-in-from-left-4"
      >
        <span
          class="font-bold text-base text-slate-900 tracking-tight leading-none uppercase"
          >Pranav</span
        >
        <span
          class="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1"
          >Diagnostic Management</span
        >
      </div>
    </div>
    ...
    <!-- Navigation Area -->
    <div
      class="flex-1 overflow-y-auto overflow-x-hidden custom-scrollbar py-6 px-4 space-y-8"
    >
      <!-- Section: General -->
      <div class="space-y-1">
        <p
    ...
    <div class="p-4 border-t border-slate-100 bg-white/50 backdrop-blur-md">
      <router-link
        v-if="!isCollapsed"
        to="/profile"
        class="mb-4 p-3 rounded-2xl bg-white border border-slate-200/60 flex items-center gap-3 shadow-sm animate-in fade-in zoom-in-95 duration-500 hover:border-primary/40 hover:bg-primary/5 transition-all"
      >
        <div
          class="w-10 h-10 rounded-xl bg-slate-50 flex items-center justify-center border border-slate-100 shrink-0"
        >
        ...
        <div class="flex flex-col min-w-0">
          ...
          <span
            class="text-[10px] text-slate-400 truncate lowercase font-medium"
            >{{ user?.email || "Fetching email..." }}</span
          >
        </div>
      </router-link>
      ...
      <div class="flex gap-2">
        ...
        >
          <LogOutIcon
            ...
            "
          />
          <span v-if="!isCollapsed" class="text-sm font-bold tracking-tight"
            >Sign Out</span
          >
        </button>
  ...
  <div
    :class="
      cn('transition-all duration-300 shrink-0', isCollapsed ? 'w-20' : 'w-72')
    "
    aria-hidden="true"
  />
...
const navMain = [
  ...
  { title: "Case Reports", url: "/case-reports", icon: CaseIcon },
  { title: "Patients", url: "/patients", icon: PatientsIcon },
  { title: "Doctors", url: "/doctors", icon: DoctorsIcon },
];

const navReports = [
  { title: "Patient Report", url: "/reports/patients", icon: ReportsIcon },
];

const navMaster = [
  { title: "Scan Types", url: "/masters/scan-types", icon: ScanIcon },
  { title: "Genders", url: "/masters/genders", icon: GenderIcon },
  { title: "Blood Groups", url: "/masters/blood-groups", icon: BloodIcon },
